...
/// How long an accepted connection may go without moving a byte, in either
/// direction, before it is abandoned.
///
/// The timeout bounds each read and each write rather than the whole exchange,
/// so it covers the longest gap between packets rather than the time a
/// transfer takes: a request of hundreds of megabytes crosses a LAN well
/// within it, while a connection that carries nothing at all costs this long
/// and no more.
