...
///
/// The width is answered here as well, and only here. An outline is stroked
/// once at a width no beam exceeds, and each vertex is drawn in from there
/// toward the contour point it was offset from — so a thickness animation
/// reaches a figure the way every other geometry animation does, at each of
/// its points, and an outline can be thick at one place around a figure and
/// gone at another.
pub fn stroke_vertex_pass(out: &mut VertexBuffers, mesh: &StrokeMesh, work: VertexWork) {
    ...
    for (i, vertex) in mesh.vertices().enumerate() {
        ...
        // The ribbon's own width, applied before anything moves the point:
        // the offset is in the figure's undisplaced coordinates, which is
        // where the contour point it is measured from lives.
