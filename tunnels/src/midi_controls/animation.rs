...
// knobs
const SPEED: Mapping = cc_mapping(48);
const WEIGHT: Mapping = cc_mapping(49);
const DUTY_CYCLE: Mapping = cc_mapping(50);
const SMOOTHING: Mapping = cc_mapping(51);
...
pub fn map_animation_controls(device: Device, map: &mut ControlMap) {
    ...
    let mut add = |mapping, creator| add_control(map, device, mapping, creator);
    ...
    add(note_on_ch0(0), |_| Animation(Set(NPeriods(0))));
    add(note_on_ch0(1), |_| Animation(Set(NPeriods(1))));
    add(note_on_ch0(2), |_| Animation(Set(NPeriods(2))));
    add(note_on_ch0(3), |_| Animation(Set(NPeriods(3))));
    add(note_on_ch0(4), |_| Animation(Set(NPeriods(4))));
    add(note_on_ch0(5), |_| Animation(Set(NPeriods(5))));
    add(note_on_ch0(6), |_| Animation(Set(NPeriods(6))));
    add(note_on_ch0(7), |_| Animation(Set(NPeriods(7))));
    add(note_on_ch0(8), |_| Animation(Set(NPeriods(8))));
    add(note_on_ch0(9), |_| Animation(Set(NPeriods(9))));
    add(note_on_ch0(10), |_| Animation(Set(NPeriods(10))));
    add(note_on_ch0(11), |_| Animation(Set(NPeriods(11))));
    add(note_on_ch0(12), |_| Animation(Set(NPeriods(12))));
    add(note_on_ch0(13), |_| Animation(Set(NPeriods(13))));
    add(note_on_ch0(14), |_| Animation(Set(NPeriods(14))));
    add(note_on_ch0(15), |_| Animation(Set(NPeriods(15))));
    ...
    // clock select
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET - 1) as u8), |_| {
        Animation(Set(ClockSource(None)))
    });
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET + 0) as u8), |_| {
        Animation(Set(ClockSource(Some(ClockIdx(0)))))
    });
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET + 1) as u8), |_| {
        Animation(Set(ClockSource(Some(ClockIdx(1)))))
    });
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET + 2) as u8), |_| {
        Animation(Set(ClockSource(Some(ClockIdx(2)))))
    });
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET + 3) as u8), |_| {
        Animation(Set(ClockSource(Some(ClockIdx(3)))))
    });
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET + 4) as u8), |_| {
        Animation(Set(ClockSource(Some(ClockIdx(4)))))
    });
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET + 5) as u8), |_| {
        Animation(Set(ClockSource(Some(ClockIdx(5)))))
    });
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET + 6) as u8), |_| {
        Animation(Set(ClockSource(Some(ClockIdx(6)))))
    });
    add(note_on_ch1((CLOCK_SELECT_CONTROL_OFFSET + 7) as u8), |_| {
...
fn event(mapping: Mapping, value: u8) -> Event {
    Event { mapping, value }
}

const fn note_on_ch0(control: u8) -> Mapping {
    Mapping {
...
const fn note_on_ch1(control: u8) -> Mapping {
    Mapping {
        event_type: EventType::NoteOn,
        channel: 1,
        control,
...
const fn cc_mapping(control: u8) -> Mapping {
    Mapping {
