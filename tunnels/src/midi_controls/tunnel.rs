...
use crate::{
    midi::{Event, Mapping, MidiOutput, cc, cc_ch0, event, note_on, note_on_ch0},
    ...
    match sc {
        ...
        ShapeMode(v) => {
            ...
            if !v.reads_marquee_knob() {
                // The marquee slides segments along a path, so a knob left
                // where the last mode put it would report a setting that
                // nothing reads.
