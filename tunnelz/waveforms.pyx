...
cpdef inline sine(float angle, float smoothing, float duty_cycle):
    angle = (angle % TWOPI)
...
cpdef inline triangle (float angle, float smoothing, float duty_cycle):
...
cpdef inline float square(float angle, float smoothing, float duty_cycle):
...
cpdef inline float sawtooth(float angle, float smoothing, float duty_cycle):
...
def sine_vector(np.ndarray[np.float_t, ndim=1] angles, np.float smoothing, np.float duty_cycle):
    cdef int size = angles.shape[0]
    cdef np.ndarray[np.float_t, ndim=1] output = np.empty(size, np.float)
    cdef size_t i

    for i in xrange(size):
        output[i] = sine(angles[i], smoothing, duty_cycle)

    return output

def sawtooth_vector(np.ndarray[np.float_t, ndim=1] angles, np.float smoothing, np.float duty_cycle):
    ...
    for i in xrange(size):
        output[i] = sawtooth(angles[i], smoothing, duty_cycle)

    return output

def triangle_vector(np.ndarray[np.float_t, ndim=1] angles, np.float smoothing, np.float duty_cycle):
    cdef int size = angles.shape[0]
    cdef np.ndarray[np.float_t, ndim=1] output = np.empty(size, np.float)
    cdef size_t i

    for i in xrange(size):
        output[i] = triangle(angles[i], smoothing, duty_cycle)

    return output

def square_vector(np.ndarray[np.float_t, ndim=1] angles, np.float smoothing, np.float duty_cycle):
    cdef int size = angles.shape[0]
    cdef np.ndarray[np.float_t, ndim=1] output = np.empty(size, np.float)
    cdef size_t i

    for i in xrange(size):
        output[i] = square(angles[i], smoothing, duty_cycle)
